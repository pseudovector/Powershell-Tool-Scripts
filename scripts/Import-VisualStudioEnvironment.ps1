...
# pseudovector
#
# Forked from https://github.com/Wintellect/WintellectPowerShell by John Robbins/Wintellect
...
#>
    ...
    (
        ...
        [ValidateSet("Latest", "2013", "2015", "2017", "2019", "2022")]
        [string] $VSVersion = "Latest", 
        [Parameter(Position=1)]
        [ValidateSet("Latest", "140", "141", "142", "143")]
        [string] $VCVersion = "Latest", 
        [Parameter(Position=2)]
        [ValidateSet("x86", "amd64", "arm")]
        [string] $Arch = ($Env:PROCESSOR_ARCHITECTURE).ToLower(),
        [String] $hostArch = ($Env:PROCESSOR_ARCHITECTURE).ToLower(),
        [string] $AdditionalOptions = ""
    )  

    # VS 2017 and later provide vswhere.exe, which can be used
    $vswherePath = ""
    ...
    # Search for VS versions with vswhere
    if (![String]::IsNullOrEmpty($vswherePath))
    {
        if ($VSVersion -eq 'Latest') {
            $vsDirectory = Invoke-CmdScript -script "$vswherePath" -parameters "-latest -prerelease -products * -property installationPath"
            $VSVersion = Invoke-CmdScript -script "$vswherePath" -parameters "-latest -prerelease -products * -property catalog_productLineVersion"
        }
        else
        {
            $verRange = switch($VSVersion)
            {
                "2013" { "[12.0,13.0)" }
                "2015" { "[14.0,15.0)" }
                "2017" { "[15.0,16.0)" }
            ...
            $parameters = "-latest -prerelease -products * -version " + $verRange + " -property installationPath"
            $vsDirectory = Invoke-CmdScript -script "$vswherePath" -parameters $parameters
        }
    }
    else
    {
        # Search for legacy versions from registry if vswhere.exe does not exist
    ...
    else
    {
        ...
        else 
        {
            ...
            $parameters = "-no_logo -arch=$Arch -host_arch=$HostArch"
            if ($VCVersion -ne "Latest")
            {
                $vcVer = switch($VCVersion)
                {
                    "140" { "14.0" }
                    "141" { "14.1" }
                    "142" { "14.2" }
                    "143" { "14.3" }
                    default { throw "Unknown version of VC++ compiler toolset!" }
                }
                $parameters += " -vcvars_ver=$vcVer"
            }
            if ($AdditionalOptions -ne "")
            {
                $parameters += " $AdditionalOptions"
            }
            Invoke-CmdScript -script "$vsDirectory\Common7\Tools\VsDevCmd.bat" -parameters $parameters -Verbose
        }
    }
}
